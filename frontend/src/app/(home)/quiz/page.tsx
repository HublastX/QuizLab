    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const params = {
                ...
                num_questions: formData.num_questions ? parseInt(formData.num_questions) : undefined,
                num_alternatives: formData.num_alternatives ? parseInt(formData.num_alternatives) : undefined
            };
                ...
                <div style={{ marginTop: '30px' }}>
                    ...
                    {data.perguntas.map((pergunta, index) => (
                        <div key={index} style={{ border: '1px solid #ccc', padding: '15px', margin: '10px 0', borderRadius: '5px' }}>
                            <h4>Pergunta {index + 1}: {pergunta.pergunta}</h4>
                            <div>
                                {pergunta.alternativas.map((alt, altIndex) => (
                                    <div 
                                        ...
                                        style={{ 
                                            padding: '8px', 
                                            margin: '5px 0',
                                            backgroundColor: alt.correta ? '#e8f5e8' : '#f5f5f5',
                                            border: alt.correta ? '2px solid #4caf50' : '1px solid #ddd'
                                        }}
                                    >
                                        <strong>{alt.letra}</strong>. {alt.texto}
                                        {alt.correta && <span style={{ color: '#4caf50', marginLeft: '10px' }}>✓ Correta</span>}
                                        {alt.explicacao && (
