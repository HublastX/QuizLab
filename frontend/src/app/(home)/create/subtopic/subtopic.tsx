...
interface SubtopicProps {
  themeId?: string;
  onSubtopicChange: (data: { id?: string; subTopic: string; description: string; isNew: boolean }) => void;
}
...
export function Subtopic({ themeId, onSubtopicChange }: SubtopicProps) {
  ...
  useEffect(() => {
    if (selectedSubTopicId) {
      const selected = subTopics.find((st) => st.id === selectedSubTopicId);
      if (selected) {
        onSubtopicChange({
          id: selected.id,
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      <div className="mt-6 text-center">
        ...
        {!themeId ? (
          <p className="text-gray-500 mt-2">Selecione um tema primeiro</p>
        ) : loading ? (
          <p>Carregando...</p>
        ) : (
          <select
